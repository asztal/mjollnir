...
data ProgramStatement
    = ModuleAssign LName (Located ModuleDecl)
    | ModuleFrom LName LName (Located ModuleDecl)
    deriving Show
...
instance Show Syntax where
    ...
    showsPrec p (CaseS (L _ x) cases y)
        ...
        where
            showCases [] = prepend ""
            showCases (x:xs) = showCase x . showCases xs
            showCase (ranges, exprs) = prepend "kostur " . showRanges ranges . prepend " þá " . showList exprs
            showRanges [] = prepend ""
            showRanges (L _ x : xs) = showRange x . showRanges xs
